...
def gerar_datas_serie():
    """Gera data inicial e final em formato dd/mm/yyyy para uso na API do Banco Central."""
    ...
    data_inicio = f"{ultimo_dia_inicio:02d}/{mes_inicio:02d}/{ano_inicio}"
    data_fim = f"01/{mes_final:02d}/{ano_final}"
...
def realiza_calculo(indice, narr):
    ...
    print("Valor do reajuste: R$",valor_final)
    return valor_final
...
def main(event):
    ...
    valor_original = float(event["inputFields"]['Valor_total'])

    print(f'Valor original do Cliente = {valor_original}')
    ...
    if tipo_reajuste == 'IGPM' and valor_original > 0.0:
        ...
        valor_do_reajuste = realiza_calculo(resultado_igpm['acumulado_12_meses'], valor_original)
        indice_final_reajuste = resultado_igpm['acumulado_12_meses']

        valor_com_reajuste = valor_do_reajuste + valor_original
        print("Valor total com o reajuste: R$", valor_com_reajuste)
        ...
        valor_do_reajuste = realiza_calculo(resultado_ipca['acumulado_12_meses'], valor_original)
    ...
    return {
        "outputFields": {
            ...
            "return_valor_do_reajuste": valor_do_reajuste,
